...
app.get("/", async (req, res) => {
  return res.send("Welcome to play-my-playlist REST api");
});
...
io.on("connection", (socket) => {
    ...
    ({ user_id, room_id, name, songs_list, song_count, video_stream }) => {
      ...
      if (!userExists) {
        ...
        if (user) {
          ...
          socket.broadcast.to(user.room_id).emit(
            "message",
            formatMessages(
              botName,
              null,
  ...
  socket.on("remove_songs", ({ name }) => {
    ...
    if (user) {
      ...
      io.to(user.room_id).emit(
        "message",
        formatMessages(botName, null, `${name} removed a song.`),
      );
    }
  });
  ...
  socket.on("game_event", ({ game_event, song_id, room_obj_id }) => {
    const user = getUser(socket.id);
    if (user) {
      io.to(user.room_id).emit("change_game_event", {
        game_event,
        song_id,
        room_obj_id,
      });
